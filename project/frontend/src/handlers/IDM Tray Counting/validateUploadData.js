export const validateUploadData = (yoloImages, yoloLabels, volume) => {
  if (yoloImages.length === 0 || yoloLabels.length === 0 || !volume) {
    return '[ERROR] Please select YOLO images, YOLO labels and volume.';
  }

  if (yoloImages.length !== yoloLabels.length) {
    return `[ERROR] Number of images (${yoloImages.length}) and labels (${yoloLabels.length}) do not match!`;
  }
  ...
  if (missingLabels.length > 0) {
    return `[ERROR] Missing label files for images: ${missingLabels.join(', ')}`;
  }
